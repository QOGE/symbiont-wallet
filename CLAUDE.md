...
- **Audit 4 (single-use address lifecycle) — COMPLETE:** Auditor: Grok Build (xAI, Composer 2.5), local direct filesystem access, single structured pass — 7 July 2026. **Not yet cross-validated by a second independent model**, unlike Audits 1–3; a second pass on the redesigned code is recommended before mainnet, per the project's general practice. Two HIGH/CRITICAL-severity design gaps found and fixed via structural redesign (not targeted patches): (1) `OnConfirmation` coupled reuse-prevention flagging with irreversible key destruction — decoupled: `OnConfirmation` now only flags SPENT at ≥ 1 confirmation; `PurgeSpentKey` is a separate, explicit, manual, irreversible method (≥ 101 confirmations); resolves reorg-after-destruction fund-loss risk and receive-vs-spend confirmation ambiguity as a side effect of the redesign. (2) `SignP2QPKInput` had no enforcement that change routed to a FRESH wallet-controlled address and no post-sign transition — both now enforced; same fix added to `SignTransaction` stub. New methods: `PurgeSpentKey`, `ListPurgeEligibleAddresses`. New keystore method: `ListByState`. 12 new tests (75/75 pass). No finding blocks mainnet activation. Triage artifact: `docs/sips/Audit_4_single_use_lifecycle_triage_summary.md`.
