...
- **Audit 4 (single-use address lifecycle) — COMPLETE:** Multi-model audit of the address state machine and signing path. Three HIGH findings resolved: (1) `OnConfirmation` coupled reuse-prevention flagging with irreversible key destruction at a high confirmation threshold — decoupled into `OnConfirmation` (flags SPENT at 1 confirmation) and `PurgeSpentKey` (optional, manual, irreversible, requires SPENT state and ≥ 101 confirmations); (2) `SignP2QPKInput` never validated that `ChangeAddr` is wallet-controlled and FRESH before signing, and never transitioned it afterward — both checks now enforced; (3) `SignTransaction` (stub) validated FRESH but never transitioned change after signing — post-sign transition added. New methods: `PurgeSpentKey`, `ListPurgeEligibleAddresses`. New keystore method: `ListByState`. 12 new tests (75/75 pass). No finding blocks mainnet activation (M1.6 / `SignTransaction` is still a stub; `SignP2QPKInput` change enforcement is now correct for production use).
