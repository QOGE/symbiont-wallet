...
| Reference production node config | `37adfd719` (QOGE/qogecoin) | `contrib/qogecoin-production.conf.example` — copy and edit before deploying a fresh mainnet node. Three documented operational lessons: (1) `changetype=bech32` — forces wallet change outputs to SegWit v0 (P2WPKH); Bitcoin-Core-derived wallets default change to Taproot once active, but Taproot exposes the raw public key on-chain at funding time, wrong for a PQC project; (2) `fallbackfee`/`paytxfee`/`mintxfee` — fee estimation silently fails on low-activity or freshly-synced chains, halting all payment processing; (3) `wallet=` persistence — a wallet loaded via `loadwallet` RPC is not reloaded on daemon restart, causing silent `-18` errors on every subsequent restart. |
