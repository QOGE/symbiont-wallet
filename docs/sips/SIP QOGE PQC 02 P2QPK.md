...
| F | Public testnet | 🔄 IN PROGRESS — `DEPLOYMENT_P2QPK` added to `CTestNetParams` (ALWAYS_ACTIVE, bit 3, `89812b7c`); `bech32_hrp = "bqt"`; `DeploymentInfo()` wired for all chains (`rpc/blockchain.cpp:1275`); `p2qpk: active: true` confirmed on both testnet and regtest. Consensus safety: `BIP9Deployment` safe defaults + explicit `NEVER_ACTIVE` in `CMainParams`/`CSigNetParams` (`09638b35`, per independent review). `address.Network` type + `bqt` HRP support added to Symbiont Wallet address package (`83bbc73`). Pending: Option A liboqs build (`depends/packages/liboqs.mk`), `nRuleChangeActivationThreshold` fix on testnet, independent BIP9 parameter review, public testnet node launch. |
