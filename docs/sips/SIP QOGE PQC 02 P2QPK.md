...
| **Audit 4 complete** (single-use address lifecycle) | — | Multi-model audit of address state machine and signing path (7 July 2026). Three HIGH findings: reorg-after-key-destruction fund loss risk (decoupled flagging/destruction); change-address reuse risk in `SignP2QPKInput` (no validation or post-sign transition); same gap in `SignTransaction` stub. All three fixed. Not a bottleneck for mainnet activation (`SignTransaction` remains a stub; `SignP2QPKInput` change enforcement is now correct for production use). |
