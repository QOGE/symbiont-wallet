...
| Retirement atomicity fix | `b093d0f` (symbiont-wallet) | `OnConfirmation` now uses `MarkSpentAndRetire` (single bbolt transaction) — PENDING→RETIRED is atomic; no crash window between `MarkSpent` and `Retire`. Found via Audit 5. |
